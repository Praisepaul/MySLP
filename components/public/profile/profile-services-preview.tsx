...
export async function ProfileServicesPreview({ therapistProfile }: { therapistProfile: EditableTherapistProfile }) {
  const activeServices = (await getServices()).filter((service) => service.active).sort((a, b) => a.order - b.order);
