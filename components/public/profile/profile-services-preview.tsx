    ...
    <section id="services" aria-labelledby="services-title" className="border-t py-20 sm:py-24">
      <div className="mx-auto w-full max-w-7xl px-4 sm:px-6 lg:px-8">
        <div className="flex flex-col gap-6 sm:flex-row sm:items-end sm:justify-between">
          ...
          <Link href="/book" className="inline-flex items-center gap-2 text-sm font-medium underline-offset-4 hover:underline">Book a session <ArrowRight aria-hidden="true" className="size-4" /></Link>
        </div>
        <div className="mt-10 grid gap-4 md:grid-cols-3">
          {activeServices.map((service) => <ServiceCard key={service.id} service={service} />)}
        </div>
        <p className="mt-6 text-xs leading-5 text-muted-foreground">{therapistProfile.acceptsOnlineAppointments ? "Online sessions are available." : "Session availability will be shown during booking."}</p>
      </div>
    </section>
  );
}
