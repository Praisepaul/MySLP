...
export function BookingDateTimePicker({ service, dates, selectedDate, selectedSlot, slotsByDate, timezone, loading, onDateSelect, onSlotSelect, onContinue, onBack, onTimezoneChange }: BookingDateTimePickerProps) {
  const selectedSlots = selectedDate ? slotsByDate[selectedDate] ?? [] : [];
  return (
    <section aria-labelledby="date-time-step-title" className="space-y-6">
      <div><p className="text-sm font-medium text-muted-foreground">Step 2 of 4</p><h2 id="date-time-step-title" className="mt-2 text-2xl font-semibold tracking-tight sm:text-3xl">Find a time that works</h2><p className="mt-2 max-w-2xl text-sm leading-6 text-muted-foreground">Choose a date and available time. Times are shown in your selected timezone.</p></div>
      ...
      <div>
        ...
        {!loading && dates.length === 0 && <div className="mt-3 rounded-2xl border border-dashed p-6 text-center text-sm text-muted-foreground">There are no available times in the next two weeks. Please try another timezone or check back later.</div>}
      </div>
      <div aria-live="polite">
        {loading ? <div className="flex min-h-32 items-center justify-center rounded-2xl border border-dashed"><div className="flex items-center gap-2 text-sm text-muted-foreground"><Loader2 aria-hidden="true" className="size-4 animate-spin" /> Checking availability…</div></div> : selectedDate ? <div><div className="flex items-center justify-between gap-4"><div><h3 className="font-semibold">Available times</h3><p className="mt-1 text-sm text-muted-foreground">{service.name} · {formatDateLabel(selectedDate)}</p></div>{selectedSlot && <span className="inline-flex items-center gap-1.5 text-xs font-medium text-primary"><Check aria-hidden="true" className="size-3.5" /> Selected</span>}</div><div className="mt-4 grid grid-cols-2 gap-2 sm:grid-cols-3 lg:grid-cols-4">{selectedSlots.map((slot) => { const selected = selectedSlot?.start.getTime() === slot.start.getTime(); return <button key={slot.start.toISOString()} type="button" aria-pressed={selected} onClick={() => onSlotSelect(slot)} className={`rounded-xl border px-4 py-3 text-sm font-medium transition-colors ${selected ? "border-primary bg-primary text-primary-foreground" : "bg-background hover:bg-muted"}`}>{formatSlotTime(slot, timezone)}</button>; })}</div></div> : !dates.length ? null : <div className="rounded-2xl border border-dashed p-8 text-center text-sm text-muted-foreground">Select an available date to see times.</div>}
      </div>
