...
export function AppointmentReschedule({ appointment, service }: AppointmentRescheduleProps) {
  ...
  const loadAvailability = useCallback(async () => {
    ...
    try {
      const response = await fetch("/api/availability", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ serviceId: service.id, timezone, dates: dateRange }), cache: "no-store" });
      ...
      Object.entries(data.slotsByDate as Record<string, AvailabilitySlotResponse[]>).forEach(([date, slots]) => {
        nextSlotsByDate[date] = slots.map((slot) => ({ start: new Date(slot.start), end: new Date(slot.end), timezone, serviceId: service.id }));
      });
  ...
  const { refresh, refreshing } = useDataSync({ revisionUrl: "/api/availability/revision", onRefresh: loadAvailability, intervalMs: 3000 });
