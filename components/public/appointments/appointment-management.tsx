...
export function AppointmentManagement({ initialAppointment }: AppointmentManagementProps) {
  ...
  return (
    <div className="bg-muted/20 py-10 sm:py-16 lg:py-24"><div className="mx-auto w-full max-w-2xl px-4 sm:px-6 lg:px-8">
      ...
      {appointment.status === "confirmed" && <div className="mt-6 space-y-3"><CalendarActions appointment={appointment} />{appointment.googleMeet?.joinUrl && <a href={appointment.googleMeet.joinUrl} target="_blank" rel="noreferrer" className={buttonVariants({ variant: "outline" }) + " hidden"}><Video aria-hidden="true" /> Join Google Meet</a>}</div>}
