...
export function ServicesManager({ initialServices }: { initialServices: Service[] }) {
  ...
  async function disable(id: string) {
    setBusy(true); setError("");
    try { const response = await fetch("/api/admin/services", { method: "PATCH", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ id }) }); const data = await response.json(); if (!response.ok) throw new Error(data.error ?? "Couldn't update service."); setItems(data.services); }
  ...
  return <>
    ...
    <ServicesList services={items} onEdit={(service) => { setEditing(service); setShowForm(true); }} onDisable={disable} onDuplicate={duplicate} busy={busy} />
