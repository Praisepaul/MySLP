...
export function AvailabilityRulesListV2({
  rules,
  editingId,
  onEdit,
  onSetActive,
    ...
    return (
      <Card>
        <CardContent className="flex min-h-48 flex-col items-center justify-center px-6 py-10 text-center">
          ...
          <h3 className="mt-4 text-base font-semibold">No availability added</h3>
      ...
      {sorted.map((rule) => {
        ...
        return (
          <Card key={rule.id} className="overflow-visible">
            <CardHeader className="flex flex-row items-start justify-between gap-4 space-y-0">
              ...
              >
                <Button
                  ...
                  variant="ghost"
                  size="icon"
                  aria-label={`More actions for ${labels[rule.dayOfWeek]} availability`}
                  aria-expanded={menu}
                  onClick={() => setOpenId(menu ? null : rule.id)}
                >
                ...
                {menu && (
                  <div
                    role="menu"
                    className="absolute right-0 z-50 mt-2 w-40 rounded-xl border bg-background p-1 shadow-lg"
                  >
                    <button
                      type="button"
                      role="menuitem"
                    ...
                    <button
                      ...
                      onClick={() => {
                        setOpenId(null);
                        onSetActive?.(rule.id, !rule.active);
                      }}
                    >
                    ...
                    <button
                      type="button"
                      role="menuitem"
                      className="block w-full rounded-lg px-3 py-2 text-left text-sm text-destructive hover:bg-destructive/10 disabled:opacity-50"
                      disabled={busy}
                      onClick={() => {
                        setOpenId(null);
                        onDelete?.(rule.id);
            ...
            {editingId === rule.id && renderEditor && (
