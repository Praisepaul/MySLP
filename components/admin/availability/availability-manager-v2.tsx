...
export function AvailabilityManagerV2({
  ...
  function saveRules(nextRulesFromForm: AvailabilityRule[]) {
    ...
    const remainingRules = replacingId
      ? rules.filter((rule) => rule.id !== replacingId)
      : rules;

    for (const rule of nextRulesFromForm) {
      const collision = remainingRules.find(
        (existing) =>
      ...
      if (collision) {
        setError(
          `${rule.dayOfWeek.charAt(0).toUpperCase()}${rule.dayOfWeek.slice(1)} already has this availability window.`,
        );
        return;
      }
    }
      ...
      <section>
        <div className="mb-4 flex flex-col gap-3 sm:flex-row sm:items-end sm:justify-between">
          <div>
            ...
            <p className="mt-1 text-sm leading-6 text-muted-foreground">
              Set your normal recurring hours. You can apply the same hours to
        ...
        <AvailabilityRulesListV2
          ...
          onSetActive={(id, active) =>
            void save(
              rules.map((rule) => (rule.id === id ? { ...rule, active } : rule)),
              exceptions,
            )
          }
          onDelete={(id) =>
      ...
      <section>
        <div className="mb-4 flex flex-col gap-3 sm:flex-row sm:items-end sm:justify-between">
          <div>
            ...
            <p className="mt-1 text-sm leading-6 text-muted-foreground">
              Use a specific date when a day differs from your normal weekly
              schedule. These changes take priority over recurring hours.
            </p>
          </div>
          <Button
            type="button"
        ...
        <AvailabilityExceptionsListV2
          ...
          onDelete={(id) =>
            void save(
              rules,
              exceptions.filter((exception) => exception.id !== id),
            )
          }
          busy={busy}
          renderEditor={(item) => (
