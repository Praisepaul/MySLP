...
function parseDateBoundary(date: string): Date {
  ...
  if (boundary.toISOString().slice(0, 10) !== date) {
    throw new Error("Invalid availability date.");
  }
...
function validateRequest(input: PublicAvailabilityRequest): { serviceId: string; timezone: string; dates: string[] } {
  ...
  if (!Array.isArray(input.dates) || input.dates.length < 1 || input.dates.length > availabilityLookaheadDays) {
    throw new Error("Please choose a valid availability range.");
  }

  const dates = input.dates.filter((date): date is string => typeof date === "string");
  if (dates.length !== input.dates.length || new Set(dates).size !== dates.length) {
    throw new Error("Please choose a valid availability range.");
  }
...
export async function getPublicBookableSlots(
  ...
  const [appointments, calendarBusy] = await Promise.all([
    ...
    getGoogleCalendarBusyIntervals(rangeStart, rangeEnd),
  ]);

  const appointmentConflicts: BookingConflict[] = appointments.map((appointment) => ({
    start: appointment.startAt,
    end: appointment.endAt,
    source: "appointment",
  }));
  const calendarConflicts: BookingConflict[] = (calendarBusy ?? []).map((interval) => ({
