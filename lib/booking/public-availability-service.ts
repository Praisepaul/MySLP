...
function parseDateBoundary(date: string): Date {
  if (!/^\d{4}-\d{2}-\d{2}$/.test(date)) throw new Error("Invalid availability date.");
  const [year, month, day] = date.split("-").map(Number);
  const boundary = new Date(Date.UTC(year, month - 1, day));
  if (boundary.toISOString().slice(0, 10) !== date) throw new Error("Invalid availability date.");
  return boundary;
}
function validateRequest(input: PublicAvailabilityRequest): { serviceId: string; timezone: string; dates: string[] } {
  if (typeof input.serviceId !== "string" || input.serviceId.length > 100) throw new Error("Please choose a valid service.");
  if (typeof input.timezone !== "string" || input.timezone.length > 100) throw new Error("Please choose a valid timezone.");
  assertTimezone(input.timezone);
  if (!Array.isArray(input.dates) || input.dates.length < 1 || input.dates.length > availabilityLookaheadDays) throw new Error("Please choose a valid availability range.");
  const dates = input.dates.filter((date): date is string => typeof date === "string");
  if (dates.length !== input.dates.length || new Set(dates).size !== dates.length) throw new Error("Please choose a valid availability range.");
  dates.forEach(parseDateBoundary);
  return { serviceId: input.serviceId, timezone: input.timezone, dates };
}
...
export async function getPublicBookableSlots(input: PublicAvailabilityRequest): Promise<PublicAvailabilityResult> {
  ...
  const [appointments, calendarBusy] = await Promise.all([
    findActiveAppointmentsOverlapping(rangeStart, rangeEnd),
    getCachedGoogleCalendarBusyIntervals(rangeStart, rangeEnd),
  ]);
  ...
  const slotsByDate = Object.fromEntries(dates.map((date) => [date, getBookableSlots({ date, service, timezone, conflicts, now: new Date() }).slots]));
