...
} from "@/lib/calendar/google-calendar-event-service";
...
export class AppointmentBookingError extends Error {
  constructor(public readonly code: "INVALID_REQUEST" | "UNAVAILABLE" | "DATABASE", message: string) {
    super(message);
...
function getBookingBuckets(startAt: Date, endAt: Date) {
  ...
  const buckets: Date[] = [];
  for (let timestamp = first; timestamp < endAt.getTime(); timestamp += bucketMilliseconds) buckets.push(new Date(timestamp));
  return buckets;
}

function validateRequest(input: { serviceId: unknown; startAt: unknown; timezone: unknown; name: unknown; email: unknown; idempotencyKey: unknown }) {
  if (typeof input.serviceId !== "string" || input.serviceId.length > 100) throw new AppointmentBookingError("INVALID_REQUEST", "Please choose a valid service.");
...
async function getCalendarConflicts(conflictStart: Date, conflictEnd: Date) {
  try {
    return await getGoogleCalendarBusyIntervals(conflictStart, conflictEnd);
  } catch {
    throw new AppointmentBookingError("UNAVAILABLE", "We couldn't verify the therapist's calendar right now. Please try again in a moment.");
  }
}

async function learnCalendarConflicts(calendarConflicts: Awaited<ReturnType<typeof getGoogleCalendarBusyIntervals>>) {
  if (!calendarConflicts?.length) return;
  const learned = await Promise.all(calendarConflicts.map((interval) => saveDiscoveredGoogleCalendarConflict({ calendarId: googleCalendarConnectionId, start: interval.start, end: interval.end })));
...
export async function createAppointment(input: {
  ...
  const requestedSlot = getBookableSlots({ date, service, timezone: input.timezone as string, conflicts, now: new Date() }).slots.find((slot) => slot.start.getTime() === startAt.getTime() && slot.end.getTime() === endAt.getTime());
  if (!requestedSlot) {
  ...
  const appointment: AppointmentDocument = {
    ...
    status: "confirmed",
    service: { id: service.id, name: service.name, durationMinutes: service.durationMinutes, online: service.online, inPerson: service.inPerson },
    patient: { name: (input.name as string).trim(), email: (input.email as string).trim().toLowerCase() },
    startAt,
    endAt,
  ...
  try {
    await client.withSession(async (session) => session.withTransaction(async (transactionSession) => {
      const locks = getBookingBuckets(startAt, endAt).map((bucketStart) => ({ bucketStart, confirmationToken }));
  ...
  try {
    ...
    await updateGoogleCalendarSyncStatus({ confirmationToken, syncStatus: event ? "synced" : "not_connected", ...(event ? { eventId: event.eventId, meetJoinUrl: event.meetJoinUrl } : {}) });
  } catch (error) {
    await updateGoogleCalendarSyncStatus({ confirmationToken, syncStatus: "failed", error: error instanceof Error ? error.message.slice(0, 500) : "Google Calendar event creation failed." });
  }
...
export async function rescheduleAppointment(input: { confirmationToken: string; startAt: unknown }) {
  ...
  try {
    if (updatedAppointment.googleCalendar?.eventId) {
      ...
      const event = await createGoogleCalendarAppointmentEvent(updatedAppointment);
      await updateGoogleCalendarSyncStatus({ confirmationToken: appointment.confirmationToken, syncStatus: event ? "synced" : "not_connected", ...(event ? { eventId: event.eventId, meetJoinUrl: event.meetJoinUrl } : {}) });
    }
  } catch (error) {
    await updateGoogleCalendarSyncStatus({ confirmationToken: appointment.confirmationToken, syncStatus: "failed", error: error instanceof Error ? error.message.slice(0, 500) : "Google Calendar event update failed." });
  }
