...
} from "@/lib/calendar/google-calendar-crypto";
import {
...
async function getAuthenticatedGoogleCalendar() {
  ...
  oauthClient.setCredentials({
    refresh_token: decryptGoogleRefreshToken(connection.encryptedRefreshToken),
  });
...
export async function getGoogleCalendarBusyIntervals(
...
): Promise<GoogleCalendarBusyInterval[] | null> {
  ...
  if (!calendarData) {
    throw new Error("Google Calendar did not return free/busy data for the configured calendar.");
  }
  if (calendarData.errors?.length) {
    throw new Error("Google Calendar could not read the configured calendar availability.");
  }
...
export async function getCachedGoogleCalendarBusyIntervals(
  start: Date,
  end: Date,
): Promise<GoogleCalendarBusyInterval[] | null> {
  const cached = await getGoogleCalendarBusyCache(start, end);
  const discovered = await getDiscoveredGoogleCalendarConflicts(start, end);

  if (cached) return [...cached, ...discovered];
  ...
  return [...busyIntervals, ...discovered];
}
